...
import static com.zaidMEx.security.user.Role.*;
...
public class SecurityConfiguration {
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    http
        ...
        .requestMatchers("/api/v1/AGENT/**").hasAnyRole(ADMIN.name(), AGENT.name())


        .requestMatchers(GET, "/api/v1/AGENT/**").hasAnyAuthority(ADMIN_READ.name(), AGENT_READ.name())
        .requestMatchers(POST, "/api/v1/AGENT/**").hasAnyAuthority(ADMIN_CREATE.name(), AGENT_CREATE.name())
        .requestMatchers(PUT, "/api/v1/AGENT/**").hasAnyAuthority(ADMIN_UPDATE.name(), AGENT_UPDATE.name())
        .requestMatchers(DELETE, "/api/v1/AGENT/**").hasAnyAuthority(ADMIN_DELETE.name(), AGENT_DELETE.name())
